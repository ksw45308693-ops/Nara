-- Reject a replay while the same tenant and normalized email still have a
-- live pending invitation. Keep the 0006 tenant-then-email advisory lock order
-- and close expired rows before checking for a live pending bearer.
...
-- CREATE OR REPLACE preserves ACLs, but restore the SECURITY DEFINER boundary
-- explicitly for upgrade safety and drift repair.
